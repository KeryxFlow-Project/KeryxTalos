"""CCXT async wrapper for OKX exchange connectivity."""
...
class OKXClient(ExchangeAdapter):
    """
    Async wrapper for OKX exchange connectivity via CCXT.
...
def get_okx_client(sandbox: bool = True) -> OKXClient:
    """Get the global OKX client instance."""
    global _okx_client
    if _okx_client is None:
        _okx_client = OKXClient(sandbox=sandbox)
    return _okx_client
